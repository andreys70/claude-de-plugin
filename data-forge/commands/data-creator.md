---
description: End-to-end creation of a net-new ETL pipeline (config, code, or both) — intake (Jira preferred, freeform spec accepted), scaffold plan, scaffold code, commit/push/PR, PRF dry-run + iterate, PRD execution, post-deploy verification, and Jira close-out (skipped if no Jira).
argument-hint: <JIRA-KEY or freeform spec>
---

You are now driving the **create workflow** for data-forge. This slash command is the orchestrator — it runs in the main session, which means you CAN spawn specialist sub-agents via the `Agent` tool. Each sub-agent does scoped work and returns a summary; you stitch the workflow together and gate on engineer approval at the two checkpoints.
...
This is the create counterpart to `/data-forge:data-issue-fix` (bugs) and `/data-forge:data-enhancement` (changes to existing pipelines). Key differences:
...
For the create flow: **`databricks-mcp`, `DAST-Orch`, and `intuit-github-mcp` are always required. `jira-mcp` is required only when a Jira key was provided** (skip the check for freeform-spec inputs). Read the skill ref and follow it exactly — toolset inspection only, no MCP calls, deferred authentication.
...
Your only MCP touchpoints are the Phase 0 toolset-presence checks. **Every other MCP interaction is delegated to a sub-agent via `Agent`.** This is by design — the sub-agents own the read/write tools for their respective MCPs:
...
This is the planning phase. Do **not** delegate to a separate planner agent — keep it here in the orchestrator.

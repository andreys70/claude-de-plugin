...
For this orchestrator (create flow): **`databricks-mcp`, `DAST-Orch`, and `intuit-github-mcp` are always required. `jira-mcp` is required only when a Jira key was provided** (skip the check for freeform-spec inputs). Read the skill ref and follow it exactly — toolset inspection only, no MCP calls, deferred authentication.
...
Your only MCP calls are the Phase 0 probes above. **Every other MCP interaction is delegated to a sub-agent via `Agent`.** This is by design — the sub-agents own the read/write tools for their respective MCPs:
